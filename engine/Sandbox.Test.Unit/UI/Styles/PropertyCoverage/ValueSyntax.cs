using Sandbox.UI;
namespace UITest.PropertyCoverage;
...
///
/// Tests whose name ends in _Gap document web-standard behaviour s&box currently
/// gets wrong/missing - they are expected to FAIL until the engine is fixed.
/// </summary>
[TestClass]
public class ValueSyntax
{
	...
	// ----------------------------------------------------------------------------
	// GAPS - viewport units (dvh/svh/lvh)
	// ----------------------------------------------------------------------------
	...
	{
		// !important is not implemented, but for length properties Length.Parse finds the unit via
		// IndexOf and parses the leading number, so the trailing " !important" is harmlessly ignored
		// and the width is still applied. (It is NOT honoured as a cascade override - just tolerated.)
		var s = new Styles();
	...
	/// NOT intercepted as a CSS-wide keyword. 'flex: initial' must still expand to grow 1 / shrink 0.
	/// </summary>
	[TestMethod]
	public void Global_FlexInitialStaysNative()
	{
		...
		Assert.AreEqual( 1f, s.FlexGrow );
		Assert.AreEqual( 0f, s.FlexShrink );
	}
