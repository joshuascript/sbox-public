...
///
/// Notes on s&box's actual parsing behaviour (verified against Styles.Set.cs):
///   - SetPadding / SetMargin have an idiosyncratic side mapping that does NOT match
///     the CSS spec for the 2 and 3 value forms. These tests assert the ACTUAL parsed
///     values that s&box produces (matching the canonical StyleSetting.cs tests), not
///     the web-standard mapping.
...
public class SpacingProperties
{
	...
	// ---------------------------------------------------------------------
	// inset (shorthand for top/right/bottom/left) - NOT IMPLEMENTED in s&box
	// ---------------------------------------------------------------------
